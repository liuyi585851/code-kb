...
def render_user_token_binding_page() -> str:
    return """<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
  ...
  <title>Code-KB Token Binding</title>
  <style>
    :root {
...
<main>
  ...
  <section class="panel">
    <div class="grid">
      ...
      <div>
        <label for="im-userid">IM userid</label>
        <input id="im-userid" autocomplete="off">
      </div>
      <div>
        <label for="user-id-hash">User id hash</label>
        <input id="user-id-hash" autocomplete="off">
      </div>
      <div>
    ...
    <div class="actions">
      <button type="button" id="derive">Derive hash</button>
...
async function deriveHash() {
  const userId = value("im-userid");
  if (!userId) {
    setStatus("im userid is required to derive hash", "error");
    return "";
  }
  const hash = await sha256Hex(userId);
  setValue("user-id-hash", hash);
  setStatus("hash derived", "ok");
  return hash;
}
function scopes() {
...
async function issueToken() {
  ...
  let userHash = value("user-id-hash");
  if (!userHash) {
    if (!value("im-userid")) {
      setStatus("im userid or user id hash is required", "error");
      return;
    }
    try {
      userHash = await deriveHash();
    } catch (error) {
      userHash = "";
      setStatus("browser hash unavailable; server will derive hash", "warn");
    }
  }
  const metadata = {source: "web_fallback_binding"};
  if (value("im-userid")) {
    metadata.im_userid = value("im-userid");
  }
  setStatus("Issuing token...", "muted");
  const response = await fetch("/auth/im/token-bindings", {
    method: "POST",
    headers: {
...
function clearForm() {
  for (const id of ["admin-token", "display-name", "im-userid", "user-id-hash"]) {
    setValue(id, "");
  }
...
document.getElementById("derive").addEventListener("click", () => deriveHash().catch((error) => setStatus(error.message, "error")));
